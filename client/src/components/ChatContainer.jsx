...
import { formatMsgAndTime } from '../lib/utils'
  ...
  messagesDummyData.map((msg,index)=>{
    return(
      <div key={index} className={`flex items-end gap=2 justify-end ${msg.senderId !== '680f50e4f10f3cd28382ecf9' && 'flex-row-reverse'}`}>
        {
          msg.image?(
            <img src={msg.image} className='max-w-[230px] boder border-gray-700 rounded-lg overflow-hidden mb-8'></img>
          ):(<p
        ...
        <div className='text-center text-xs'>
          ...
          <p className='text-gray-500'>{formatMsgAndTime(msg.createdAt)}</p>
        </div>
